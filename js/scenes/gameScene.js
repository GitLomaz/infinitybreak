let gameScene = new Phaser.Class({
  ...
  preload: function () {
    ...
    this.load.spritesheet("asteroid_2", "images/asteroid_2.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
  ...
  create: function () {
    ...
    scene.anims.create({
      ...
      frames: scene.anims.generateFrameNumbers("orb", {
        frames: [
          0, 1, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4,
          2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3, 4, 2, 3,
    ...
    scene.anims.create({
      ...
      frames: scene.anims.generateFrameNumbers("boss", {
        frames: [
          8, 9, 10, 11, 10, 11, 10, 11, 10, 11, 10, 11, 10, 11, 10, 11, 12, 13,
        ],
      }),
      frameRate: 8,
      repeat: 0,
    ...
    this.add.image(0, 0, "header").setOrigin(0).setDepth(1);
    this.add
      .image(0, GAME_HEIGHT - 57, "footer")
      .setOrigin(0)
      .setDepth(1);
    ...
    this.gameVars.nixieDigets = [];
    for (let i = 0; i < 3; i++) {
      let nixie = this.add
        .image(484 + i * 24, 638, "nixieDigets")
        .setOrigin(0)
        .setDepth(2);
      this.gameVars.nixieDigets.push(nixie);
    }
    for (let i = 0; i < 3; i++) {
      let nixie = this.add
        .image(564 + i * 24, 638, "nixieDigets")
        .setOrigin(0)
        .setDepth(2);
      this.gameVars.nixieDigets.push(nixie);
    }
    for (let i = 0; i < 3; i++) {
      let nixie = this.add
        .image(644 + i * 24, 638, "nixieDigets")
        .setOrigin(0)
        .setDepth(2);
      this.gameVars.nixieDigets.push(nixie);
    }
    for (let i = 0; i < 3; i++) {
      let nixie = this.add
        .image(724 + i * 24, 638, "nixieDigets")
        .setOrigin(0)
        .setDepth(2);
      this.gameVars.nixieDigets.push(nixie);
    }

    this.gameVars.nixieCommas = [];
    for (let i = 0; i < 3; i++) {
      let nixie = this.add
        .image(552 + i * 80, 662, "nixieCommas")
        .setOrigin(0)
        .setDepth(2);
      this.gameVars.nixieCommas.push(nixie);
    }
  ...
  update: function () {
    ...
    if (this.gameVars.ship.alive) {
      ...
      if (!this.stageTransition) {
        ...
        if (this.gameVars.cursors.up.isDown || this.gameVars.wasd.up.isDown) {
